...
public class BestHumanMovePercent {
	...
	public static void main(String[] args) throws IOException {
		...
		//Move is in the answers file.  subtract 1, divide by 8 for row.  subtract 1 mod 8 for column.
		//Find board in CNNData that corresponds to that board
		//add original board's visitation and new board's visitation
		int correct = 0;
		int boards = 0;
		...
		ArrayList<Integer> timesSeen = new ArrayList<Integer>();
		for(int i = 0;i<numFiles;i++){
			...
			timesSeen.add(Integer.parseInt(scan.nextLine()));
			bools[i]=true;
			...
			for(String s:moves.get(i)){
				...
				for(int j = 0;j<numFiles;j++){
					if(turns[i]+1==turns[j]&&sameBoard(newBoard, arr, j)){
						...
						if(move!=0){
							correct+=timesSeen.get(j);
							boards += timesSeen.get(i);
							size++;
						}
					}
				}
